...
pacman::p_load(tidyverse,ggplot2,sf,rmapshaper,sp)
...
covered.shp = ethnies.shp %>% filter(ethnic_id %in% covered.dta$ethnic_id) %>%
  left_join(covered.dta %>% select(ethnic_id,length))

ggplot() +
  geom_sf(data = ethnies.shp, fill = "gray", color = NA) +
  geom_sf(data = covered.shp, mapping = aes(fill = length), color = "gray", lwd=0.1) +
